...
async def get_pending_requests(db_session: AsyncSession):
    """
    Получает все заявки со статусом "pending" (неподтвержденные)
    
    :param db_session: Асинхронная сессия SQLAlchemy
    :return: Список объектов User с неподтвержденными заявками
    """
    # Запрос для получения всех пользователей со статусом "pending"
    result = await db_session.execute(select(User).where(User.status == "pending"))
    
    # Получаем все заявки из результата
